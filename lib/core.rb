...
module WS
  ...
  class WSLayout
    ...
    #auto_layout時の動作が定義されているtypeの配列
    #各々の動作は同名のインスタンスメソッドを参照
    @@types = [:hbox, :vbox, :hbox_ex, :vbox_ex, :left, :right, :hcenter, :left_ex, :right_ex, :hcenter_ex, :top, :bottom, :vcenter, :top_ex, :bottom_ex, :vcenter_ex]
    ...
    def add(o, rsw=nil, rsh=nil)
      ...
      w = (o.resizable_width ? o.min_width : o.width)
      h = (o.resizable_height ? o.min_height : o.height)
      if [:hbox, :hbox_ex, :left, :right, :hcenter, :left_ex, :right_ex, :hcenter_ex].include?(@type)
        @min_width += w
        @min_height = [@min_height, h].max
      else
        @min_width = [@min_width, w].max
        @min_height += h
      end
    end
